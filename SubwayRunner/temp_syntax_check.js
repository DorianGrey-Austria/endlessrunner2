        ...
        console.log('📴 Supabase temporarily disabled - using localStorage for highscores');
        ...
        function showGameOverMenu(isVictory) {
                    ...
                    <div style="margin: 20px 0; padding: 15px; background: rgba(0,0,0,0.2); border-radius: 10px;">
                        ...
                        <table style="width: 100%; margin-top: 10px;">
                            ...
                            ${highscores.slice(0, 5).map((entry, index) => `
                                <tr style="${entry.score === currentScore ? 'background: rgba(255,215,0,0.2); font-weight: bold;' : ''}">
                                    <td style="padding: 5px;">${index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : index + 1}</td>
            ...
            if (isVictory) {
                menu.innerHTML = `
                    <h1>🎉 VICTORY! 🎉</h1>
                    <h2>Du hast ${gameState.currentRound > 1 ? `${gameState.currentRound} Durchgänge` : '60 Sekunden'} überlebt!</h2>
        ...
        async function loadGestureModule() {
            // Gesture control removed for stability
